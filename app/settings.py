...
def get_config_dir() -> Path:
    """Get the configuration directory for EZ IDE"""
    if os.name == 'nt':  # Windows
        config_dir = Path(os.environ.get('APPDATA', '')) / 'EZ-IDE'
    else:  # Linux/macOS
        config_dir = Path.home() / '.config' / 'ez-ide'
...
@dataclass
class EZSettings:
    """EZ interpreter settings"""
    interpreter_path: str = ""  # Empty = auto-detect from PATH
...
class IDESettings:
    ...
    ez: EZSettings = field(default_factory=EZSettings)
...
class SettingsManager:
    ...
    def _apply_settings(self, data: dict):
        ...
        if 'ez' in data:
            for key, value in data['ez'].items():
                if hasattr(self.settings.ez, key):
                    setattr(self.settings.ez, key, value)
